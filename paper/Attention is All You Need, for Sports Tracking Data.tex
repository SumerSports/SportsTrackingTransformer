...
\citet{TheZoo2020} is a research group developed a model architecture that won the 2020 NFL Big Data Bowl in Kaggle challenge. This victory, along with the Big Data Bowl's growing prominence, led to The Zoo Architecture (TZA) becoming a de-facto equivariant deep learning approach used by entrants in following competitions. Furthermore, this architecture powers models and stats distributed by the NFL's advanced wing \textit{Next Gen Stats}.
In their submission, they identified the importance of designing for the player-order equivariance problem but discovered that their custom equivariant TZA design \textit{outperformed} Transformers with Multi-Head Attention. TZA, as shown in Figure \ref{fig:Zoo_Model_Architecture}, relies on feature engineering vectors pairwise between each offensive and defensive player, and then operating dense layers over each pairwise vector \textit{independently}, with pooling operations to eventually reduce the dimensionality into one final prediction.
\begin{figure}[h]
    \centering
...
\end{figure}
While not explicitly cited, we find many similarities between TZA and the DeepSet approach. We expect TZA to be architecturally inferior for similar reasons:
\begin{itemize}
...
\item Similarity to tasks that previous equivariant architectures (e.g., TZA) were designed for, allowing for meaningful comparisons
...
For our experiments, we treated each frame as an independent input, although the tackle location labels are unique at the play level. We split the data into training, validation, and test sets with a 70/15/15 ratio based on unique plays. This resulted in:
\begin{itemize}
...
We conducted a grid search over learning rates, model sizes, and number of layers for both models, while keeping batch size and dropout fixed. This approach allows us to isolate the impact of architectural differences on model performance.  The results are from the best model picked over the grid search.
...
Then TZA model $g: (\mathbb{R}^{10})^{10\cdot11} \rightarrow \mathbb{R}^2$ applies successive MLPs to interaction vectors independently, only pooling across to reduce dimensionality.

\section{Results}
\begin{table}[h]
	\caption{Test Set Event-Frame Performance Comparison (Mean Squared Error)}
	\centering
	\begin{tabular}{|c|c|c|c|c|c|c}
	\hline
    \textbf{event} & \textbf{n} & \textbf{Transformer} & \textbf{Zoo } & \textbf{\% diff} \\
    \hline
    ...
    \hline
     out of bounds & 544 & 3.7 & 13.0 & 64.8 \\
    \hline
     tackle & 2994 &  1.5 & 9.2 & 71.3\\
    \hline
	\end{tabular}
	\label{tab:event_frame_results_comparison}
\end{table}
Our evaluation on a held-out test set reveals that the Transformer model outperforms TZA by 20.4\% in our score of Mean Average Error (MAE).
...
Notably, both models perform comparably for early-play events such as ball snap and handoff. TZA's strong performance in these situations aligns with its original optimization for handoff-related tasks, demonstrating its effectiveness in specific, well-defined scenarios.
...
This pattern is continued when we plot score based on temporal distance from the frame of tackle (Figure \ref{frame_diff}). Figure \ref{fig:CMC_big_run} shows a visual example of the Transformer generalizing to unseen frames in the test set significantly better than TZA is able to.

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{chart frame diff.png}
    \caption{Plot showing how the models perform comparably early in plays but The Zoo Model fails to generalize as we arrive in more varied situations}
    \label{fig:frame_diff}
\end{figure}

\begin{figure}[t]
    \centering
    \includegraphics[width=0.75\linewidth]{cmc_bigrun.png}
