...
def predict_model_as_df(model: LitModel, dataloaders: Dict[str, DataLoader], device=0, ckpt_path=None) -> pl.DataFrame:
    pred_dfs = []
    for split, dataloader in dataloaders.items():
        preds = Trainer(devices=[device], logger=False).predict(model, dataloaders=dataloader, ckpt_path=ckpt_path)
        ...
        pred_df = pl.DataFrame(
                    {
                        ...
                        "mirrored": ds_keys[:, 2],
                        "frameId": ds_keys[:, 3],
                        "tackle_x_pred": preds[:, 0].round(2),
                        "tackle_y_pred": preds[:, 1].round(2),
                    }
                ).with_columns(
...
def train_model(
    model_type, batch_size, hidden_dim, num_layers, learning_rate, dropout, use_play_features=False, device=0, dbg_run=False
):
    # load datasets
    ds_dir = DATASET_DIR / f"{model_type}_{'play' if use_play_features else 'no_play'}"
    ...
    logger = TensorBoardLogger(
        ...
        name=f"{model_type}_{'play' if use_play_features else 'no_play'}",
        log_graph=False,
    ...
    trainer = Trainer(
        ...
        callbacks=[
            ...
            callbacks.ModelCheckpoint(monitor="val_loss", save_top_k=1, filename="{epoch}-{val_loss:.0f}"),
            callbacks.ModelSummary(max_depth=3),
        ],
    )

    # print(f"Training {model_type}_{'play' if use_play_features else 'no_play'} model")
    ...
    pred_dataloaders = {
        ...
        "val": DataLoader(val_ds, batch_size=batch_size, shuffle=False, num_workers=30),
        "test": DataLoader(test_ds, batch_size=batch_size, shuffle=False, num_workers=30),
    }
    best_model_path = trainer.checkpoint_callback.best_model_path
    model_pred_df = predict_model_as_df(lit_model, device=device, dataloaders=pred_dataloaders, ckpt_path=best_model_path)
...
def main(args):
    # for play_features in [False, True]:
        # lr = 1e-04 if args.model_type == "zoo" else 1e-03
    lr = 1e-03
    layers = [1, 2, 4] #, 8]
